...
export default function PhotoCapture({ onPhotoTaken, disabled, compact }: PhotoCaptureProps) {
  ...
  // Mode compact (dans la barre secondaire sous le bouton "Décrire")
  if (compact) {
    return (
      <>
        <input
        ...
        <button
          onClick={handleClick}
          disabled={disabled}
          className="flex-1 h-12 bg-[#1E3A5F] text-white rounded-xl font-medium text-sm flex items-center justify-center gap-2 active:bg-[#162d4a] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
