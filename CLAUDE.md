...
- **`AudioRecorder` forçait `audio/webm;codecs=opus` (Session 8) → iPhone HS depuis le 04/07** — le `new MediaRecorder` imposait webm/opus + `audioBitsPerSecond:32000` + `channelCount:1` **sans `isTypeSupported()`** : sur Safari/iOS (webm inconnu) la capture partait muette → notes vides **sur le terrain (Hendrix filme sur iPhone)**. Le bug est resté invisible car (a) le rapport se génère quand même et (b) l'hallucination Whisper « Sous-titrage Société Radio-Canada » masquait le silence. Fix Session 10 (`0b1f0dd`) : détection du conteneur supporté (webm→mp4→défaut) + `start(1000)` + drop `channelCount`. **Leçon : toute API navigateur (MediaRecorder, codecs) se teste avec un feature-detect + repli ; jamais de mimeType codé en dur.**
- **iCloud a rendu le repo local INUTILISABLE (Session 10)** — la mise en garde « projet sur iCloud » est passée de « lenteurs » à **bloquant total** : fichiers déchargés → Read/`cat` renvoient 0 octet (alors que `wc -c` voit la taille), `file` dit *empty*, et **`git status` plante** (`short read while indexing`, exit 138). `brctl download` n'a rien rapatrié (Mac hors-ligne d'iCloud). **Contournement qui marche : lire via les objets Git (`git show HEAD:fichier`) + déployer par l'API GitHub Contents (`gh api --method PUT …/contents/…`, commit direct sur `main` → build Vercel).** Vraie solution = sortir le projet d'iCloud (voir §4).

### Bugs connus
- **Capture audio sur Safari/iOS** — cassée depuis le 04/07 (Session 8, mimeType webm forcé), **corrigée Session 10** (`0b1f0dd`, repli mp4). ✅ iPhone confirmé OK par Julien.
- **Chrome desktop du Mac (poste de Julien) capte du silence** — problème **LOCAL** (permission micro macOS pour Chrome, ou mauvaise entrée Son), **PAS l'app** : l'iPhone marche. À vérifier côté Julien (Réglages Système → Confidentialité → Microphone → Chrome ; jauge d'entrée Son).
- Le bug critique de génération (modèle retiré, 16/06/2026) est corrigé et protégé par repli auto + alerte.

---

## 4. PROCHAINES ÉTAPES

1. **🔴 CRITIQUE — Déplacer le projet hors d'iCloud** (~/Developer) : en Session 10, iCloud a rendu le working tree **totalement inutilisable** (Read = 0 octet, `git status` plante). Tant que c'est sur `~/Desktop`, tout dev local passe par des contournements Git/GitHub API. C'est devenu le blocage n°1.
2. **Rendre l'extension d'upload audio dynamique** — `app/chantiers/[id]/visite/visite-client.tsx` nomme encore le fichier `.webm` en dur (`'audio/webm'`) alors que Safari produit du `mp4` depuis le fix Session 10. OK aujourd'hui (Groq sniffe le contenu), mais à dériver du `blob.type` pour du 100 % propre.
3. **Renommer `lib/openai.ts`** → `lib/anthropic.ts` (cohérence — toujours pas fait, source de confusion confirmée)
4. **Tester le flux complet** sur mobile réel (iPhone ✅ Session 10 + Android)
...
*Dernière mise à jour : 2026-09-10 — **Session 10 (dépannage prod à distance — repo local cassé par iCloud)** : toute la session menée SANS working tree utilisable (voir §3 « iCloud a rendu le repo local INUTILISABLE »). Contournement : lecture du code via les objets Git (`git show HEAD:…`) + déploiements par l'**API GitHub Contents** (`gh api --method PUT repos/julien118/visite-technique-ionnyx/contents/…` : SHA courant → payload base64 → commit direct sur `main` → build Vercel auto), jamais le working tree. **(1) Mot de passe** — reset forcé de `julien@ionnyx.fr` via l'**Admin API GoTrue** (`PUT /auth/v1/admin/users/{id}`, clé `service_role` du `.env.local`, PAS le dashboard) + login vérifié OK. Le flow « mot de passe oublié » de l'app n'est pas branché (le lien retombe sur le login → pas de page « saisir un nouveau mdp »). Valeur du mdp NON consignée (secret). **(2) Fix transcription — hallucination Whisper** (`2f800b3`, `app/api/transcribe/route.ts`) : sur audio muet, Groq Whisper « bouche le silence » avec des génériques de sous-titres (« Sous-titrage Société Radio-Canada », « Merci d'avoir regardé cette vidéo », Amara.org…). Ajout d'un filtre `nettoyerTranscription()` (normalisation ASCII NFD + `[^\x00-\x7f]`, set de phrases exactes + marqueurs uniques `radio canada`/`amara`/`soustitreur`/`sous titres realises`) qui renvoie du vide, + `temperature: 0`. Testé 10/10. ⚠️ Piège au test : normaliser en remplaçant les séparateurs (`-`) par une **espace** (sinon « Radio-Canada » → « radiocanada » collé → le filtre rate) et en supprimant les apostrophes. **(3) VRAI bug de capture révélé (cas B)** : une fois l'hallucination filtrée, la voix n'était toujours PAS captée → cause = régression Session 8 dans `components/AudioRecorder.tsx` (mimeType webm/opus forcé sans `isTypeSupported()` + `channelCount:1`), **iPhone HS depuis le 04/07**. Fix (`0b1f0dd`) : détection du conteneur supporté (webm→mp4 Safari→défaut), `getUserMedia({audio:true})`, `start(1000)` (timeslice), type de blob réel, warn si blob < 1 Ko. Opus 32 kbps gardé uniquement si conteneur opus retenu. **Vérifié par Julien : iPhone ✅ ; Chrome desktop du Mac reste muet = souci LOCAL micro/permission macOS, pas l'app** (voir §3 Bugs connus). Suite propre : rendre l'extension d'upload dynamique (§4). **(4) Clarif Costructor** : sur ce déploiement `COSTRUCTOR_CIBLE=hendrix` (défaut code + `.env.local`) → tout push de devis va sur le compte Costructor d'**HENDRIX/MTC37** (`COSTRUCTOR_API_KEY_HENDRIX`), PAS celui de Julien (`COSTRUCTOR_API_KEY` = « ancien bac à sable de test » selon `lib/costructor-compte.ts`) ; choix piloté par l'env du déploiement (pas par l'utilisateur connecté), garde-fou `assertCompteCible()`. Valeur prod exacte non lue (classifieur bloque le dump `vercel env pull` des secrets prod) → conclusion tirée du défaut code + local + commentaire « BASCULE PRODUCTION 02/07 = HENDRIX ». — **Session 9 (spin-off « version démo » + ménage tickets)** : deux volets. **(1) Spin-off démo commerciale** — PRD exhaustif AUTOPORTANT pour une version DÉMO re-brandable du Système 30 Secondes (à dérouler en RDV prospect), livré dans un **projet SÉPARÉ** `~/Desktop/SYSTÈME IA - IONNYX/SYSTÈME 30 SECONDES/PRD.md` (PAS dans ce repo). Reprend le flux Hendrix complet (capture vocale+photo → rapport 30 s → devis IA → **métrés vocaux** → **vrai push Costructor**) et le rend personnalisable en 30 s depuis l'UI : **nom entreprise + logo (upload bucket `logos`) + métier** DYNAMIQUES → prompts IA PARAMÉTRÉS `{entreprise, metier}`, **catalogues seed par métier** (`lib/config-demo.ts` ; le push Costructor est en **lignes libres** → indépendant du catalogue du compte), logo PDF dynamique. Décisions Julien : Costructor = vrai push vers SON compte premium (clé unique `COSTRUCTOR_API_KEY`) + **repli visuel hors-ligne** ; **reset 1 clic + chantier de secours** pré-rempli ; **exclus du périmètre démo** : pCloud (spécifique Hendrix), tickets, Telegram/observabilité, assistant « second cerveau ». Le PRD embarque en verbatim les 3 prompts (rapport / proposer / métrés), les types, le schéma SQL+RLS+buckets, le SDK Costructor (centimes, points de base, IDs d'unités, lignes libres, idempotence) et les pièges (build `--webpack`, ESLint flat, PDF WinAnsi, favicon IONNYX immuable). **(2) Ménage prod** — supprimé le ticket de TEST « Demander à Julien » (« Impossible d'ouvrir l'interface ») dans la base MTC37. ⚠️ Reconfirmé : **le MCP Supabase de session n'a PAS accès au projet MTC37** (`xuprrfhxwpkyhucgmqmg` ; ne voit que IONNYX interne / DÉMO VT / IONNYX) → pour opérer la base MTC37, passer par la clé `service_role` du `.env.local` en API REST (`/rest/v1/…`) ; le DELETE prod est bloqué par le classifieur tant que la cible exacte n'est pas confirmée à l'utilisateur (voir mémoire `reference-supabase-mtc37-acces`). — **Session 8 (audit + optim performance, ✅ DÉPLOYÉE EN PROD le 04/07)** : baseline chiffrée (`AUDIT_PERF_MTC37.md`) puis 12 leviers (branche `perf/optim-mtc37-20260703` → mergée main → prod, fonctions confirmées **[dub1]**), **+ fix PDF `d34bb3f`** : un `≈` généré par l'IA cassait des lignes entières du PDF (encodage WinAnsi jsPDF → texte espacé débordant) → `assainirPourPdf()` au rendu ; et le PDF pesait 8,1 Mo (photos pleine réso, > limite réponse Vercel 4,5 Mo → minutes de spinner) → images en parallèle via `_next/image?w=1200` + `Accept: image/jpeg` (~184 Ko/photo) → PDF ~1 Mo en secondes. Détail des leviers : **streaming SSE de la génération** (28-60 s de silence → premier signal < 2 s, progression réelle, maxDuration 60→120), generate-report 8→3 RT DB (Promise.all + upsert + `after()`), **audio opus 32k mono** (880→232 Ko/min, transcription identique vérifiée) + **envoi unique** (`/api/transcribe` accepte `{path}`, relit le bucket côté serveur), **file de captures non bloquante** (items optimistes `temp-…`, vignette < 1 s, boutons jamais gelés, garde sur « Générer le rapport »), next/font, loading.tsx partout, vignettes **next/image** (photos stockées 0,6-2 Mo mesurées → ~30-80 Ko affichés ; transform Supabase = 403 sur le plan actuel), prompt caching, **`regions: ["dub1"]`** (iad1 confirmé avant ; Supabase ≈ eu-west-1 à confirmer au dashboard AVANT deploy). Mesures clés : gen bloquante 28,2 s à 68 tok/s (TTFT stream 1,27 s) ; Groq ~1 s pour 75 s d'audio ; Lighthouse /login 76→90, Speed Index 8,8→0,8 s. Résultats/actions/risques/checklist : `AUDIT_PERF_MTC37_RESULTATS.md`. ⚠️ Post-deploy : recharger les onglets ouverts (contrat SSE de generate-report changé) ; si le plan refuse maxDuration=120 → 60. Constat : la table `generation_logs` n'existe dans aucune migration (insert silencieusement ignoré). — **Session 7** (audit sécurité + upgrade Next 16) : audit sécurité complet (`AUDIT_SECURITE_MTC37.md`) puis remédiation déployée en prod. Correctifs : **IDOR service_role** sur `export-pdf`/`generate-report`/`pcloud-upload-rapport` → client session + RLS (les routes qui utilisaient `SUPABASE_SERVICE_ROLE_KEY` sans `getUser()` fuyaient/écrivaient sans contrôle car le middleware exclut `/api`) ; `transcribe` exige une session (abus coût Groq) ; `client-error` plafonné (anti-spam Telegram contournable) ; cron/relances/model-health/usage-digest en **fail-closed** (secrets déjà dans Vercel) ; **audio en chemin storage** au lieu d'un lien signé 1 an (jamais rejoué → M5). **Upgrade Next 14→16 + React 18→19 + ESLint 8→9** (corrige les CVE HIGH Next) — voir gotchas §3 (build `--webpack`, ESLint flat native). Reste documenté non fait : chiffrement token pCloud (M4), bucket photos privé (L1, casserait IA/PDF/Telegram), renommer `middleware.ts`→`proxy.ts`. — **Session 6** : personnalisation du rapport PDF (logo MTC37 en en-tête, parité ATG, **footer « Rapport généré par… » retiré**, nom de fichier `compte-rendu-<client>-<date>.pdf`), salutation contextuelle « Bonjour Hendrix » (heure, côté client), le tout déployé en prod. Deploy fait depuis un **git worktree isolé** pour ne pas embarquer le WIP non commité de Julien. Gotchas : l'outil Edit normalise les échappements Unicode → classe `[^\x00-\x7f]` pour dé-accentuer ; PDF rapport = jsPDF serveur (pas html2canvas). — **Session 5** : fix prod critique (modèle Sonnet 4 retiré → `claude-sonnet-4-6`) + résilience modèle (chaîne de repli + canari), export pCloud documenté, **digests usage/coût $/€ + alertes modèle & erreurs sur Telegram** (multi-tenant, voir `SURVEILLANCE.md`), rotation des clés Anthropic + Groq vérifiée (dev + prod). — Session 4 : déploiement GitHub/Vercel, header dynamique, suppression chantier, filtres/recherche, refonte design liste.*
